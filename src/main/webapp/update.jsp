...
<%
	...
	diary = new DiaryDAO().getDiary(userID, dDate);
...
<div class="modal_wrap" style= "border-radius:20px;">
    ...
    <div>
	    ...
	    <div>
	        <form action = "update.jsp?dDate=<%= dDate%>&diarySticker=<%= diary.getDiarySticker()%>&diaryTitle=<%= dairyT%>&diaryContent=<%= dairyC%>"
	         method ="get">
		...
		<div class="card-body">
      		<form method="post" action="updateAction.jsp">
				<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd" id="diaryfrom">
					<tbody>
						<tr>
							<td><input type="text" class="form-control" name="diaryTitle" maxlength="10" value ="<%= dairyT%>"></td>
						</tr>
						<tr style="background-color: #f2f2f2">
							<input type="hidden" name="diarySticker" value="<%=diary.getDiarySticker()%>">
							<td style= "text-align: right;">
							<%
							if(request.getParameter("diarySticker").equals("null")){ %>
								<p></p>
							<%}else{ %>
								<img id="stickerImg" src="<%=request.getParameter("diarySticker")%>">&nbsp;
							<%} %>
						...
						<tr>
							<td><textarea class="form-control" name="diaryContent" maxlength="2048" style="height: 350px;"><%= dairyT%></textarea></td>
						</tr>
					</tbody>
				</table>
				<input type="hidden" name="diaryDate" value ="<%= dDate%>">
